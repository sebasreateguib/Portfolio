...
export function FileSystem({
  ...
  const [openOnSingleClick, setOpenOnSingleClick] = React.useState(false)

  React.useEffect(() => {
    if (typeof window === "undefined" || typeof window.matchMedia !== "function")
      return
    const media = window.matchMedia("(pointer: coarse)")
    const sync = () => setOpenOnSingleClick(media.matches)
    sync()
  ...
  return (
    <div
      ...
      className={cn(
        "flex h-[480px] min-h-0 flex-col overflow-hidden rounded-xl border border-border/60 bg-[#0d0d0d] text-foreground outline-none",
        className
      )}
    >
      <FileSystemIconSpriteSheet />
      <div className="relative grid h-12 shrink-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 border-b border-white/8 bg-[#161616] px-2">
      ...
      {hasActiveFilters ? (
        <div className="flex shrink-0 flex-wrap items-center gap-1 border-b border-white/8 bg-[#121212] px-2 py-1.5 text-xs text-muted-foreground">
          {filters.map((filter) => {
      ...
      <div
        aria-live="polite"
        className="flex h-8 shrink-0 items-center justify-center gap-1 border-t border-white/8 bg-[#161616] px-3 text-xs text-muted-foreground"
      >
        <span>
...
type FileSystemViewProps = {
  ...
  /** Touch / coarse pointer: single tap opens instead of only selecting. */
...
function FileSystemIconsView({
  ...
  return (
    <ScrollArea
      ...
      viewportClassName="bg-[#0d0d0d] p-5 touch-pan-y [-webkit-overflow-scrolling:touch] max-sm:p-3"
      viewportProps={{
        onClick: (event) => {
          ...
          {visibleEntries.map((entry) => {
            ...
            return (
              <button
                ...
                onDoubleClick={() => {
                  if (!openOnSingleClick) onOpen(entry)
                }}
                onKeyDown={(event) => {
                  if (event.key === "Enter") onOpen(entry)
                }}
                className="group flex h-[6.375rem] flex-col items-center gap-1.5 outline-none max-sm:min-h-11"
              >
                <span
                  className={cn(
